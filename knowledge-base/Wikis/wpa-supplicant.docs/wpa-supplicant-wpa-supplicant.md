...
# Unicast IP packet in L2 multicast frame filtering
# Allow turning off IP unicast in L2 multicast frame filtering, which is enabled
# by default.
...
# frames when the AP supports proxy ARP even on networks that aren't Passpoint.
# If set to 0 (the default), this is only done on Passpoint networks.
