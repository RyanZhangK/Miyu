...
**-L**
Force Log Zeroing. Forces **xfs_repair** to zero the log even if it is dirty (contains metadata changes). When using this option the filesystem will likely appear to be corrupt, and can cause the loss of user files and/or data. See the **DIRTY LOGS** section for more information.
...
**-n**
No modify mode. Specifies that **xfs_repair** should not modify the filesystem but should only scan the filesystem and indicate what repairs would have been made. This option cannot be used together with **-e**.

**-P**
Disable prefetching of inode and directory blocks. Use this option if you find **xfs_repair** gets stuck and stops proceeding. Interrupting a stuck **xfs_repair** is safe.
...
**-e**
If any metadata corruption was repaired, the status returned is 4 instead of the usual 0. This option cannot be used together with **-n**.
