...
After that, once the media class of a device node has been selected for a particular stream node, and there are more than 1 device node matching such media class, WirePlumber will select one based on a set of priorities:
...
Apart from the above properties, there are also global settings for the linking policy. See `config_settings` for more information, the linking settings are prefixed with `linking.`.
