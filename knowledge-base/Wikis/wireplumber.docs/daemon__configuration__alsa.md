...
The ALSA monitor is enabled by default and can be disabled using the `monitor.alsa` `feature <config_features>` in the configuration file.
...
A longer, user-friendly name of the device. This will show up in most user interfaces as the device's name.
...
Type  
array of strings (example: `["FL", "FR", "LFE", "FC", "RL", "RR"]`)
...
Type  
array of strings (example: `[ "PCM", "DTS", "AC3", "EAC3", "TrueHD", "DTS-HD" ]`)
