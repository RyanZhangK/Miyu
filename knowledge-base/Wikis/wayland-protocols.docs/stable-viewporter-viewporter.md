...
- `get_viewport`: extend surface interface for crop and scale
  ...
  Arguments: `id` (new_id) - the new viewport interface id; `surface` (object) - the surface
...
- `set_source`: set the source rectangle for cropping
  ...
  Arguments: `x` (fixed) - source rectangle x; `y` (fixed) - source rectangle y; `width` (fixed) - source rectangle width; `height` (fixed) - source rectangle height
- `set_destination`: set the surface size for scaling
  ...
  Arguments: `width` (int) - surface width; `height` (int) - surface height
