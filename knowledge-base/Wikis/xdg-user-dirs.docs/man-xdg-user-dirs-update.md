...
`xdg-user-dirs-update` updates the current state of the users `user-dirs.dirs`. If none existed before then one is created based on the system default values, or falling back to the old non-translated filenames if such directories exists. The list of old directories used are: `$HOME/Desktop`, `$HOME/Templates` and `$HOME/Public`.
...
`--force`
Update existing `user-dirs.dirs`, but force a full reset. This means: Don't reset nonexisting directories to HOME, rather recreate the directory. Never use backwards compatible non-translated names. Always recreate `user-dirs.locale`.
...
\<NAME\> should be one of the following: DESKTOP, DOCUMENTS, DOWNLOAD, MUSIC, PICTURES, PROJECTS, PUBLICSHARE, TEMPLATES, VIDEOS
