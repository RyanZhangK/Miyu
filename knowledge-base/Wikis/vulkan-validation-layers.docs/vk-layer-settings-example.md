...
khronos_validation.gpuav_acceleration_structures_builds = true
...
khronos_validation.gpuav_copy_memory_indirect = true
...
# Maximum Indices
# =====================
# Number of indices held in the buffer used to index commands and validation resources (Max number of draws/dispatches in a command buffer that can be tracked). Can be lowered to save memory.
khronos_validation.gpuav_max_indices_count = 8192

# Mesh Shading SPIR-V instructions
# =====================
# Enable shader instrumentation on VK_EXT_mesh_shader
khronos_validation.gpuav_mesh_shading = true
...
# Shader Sanitizer
# =====================
# Things invalid to do in SPIR-V but don't have a dedicated check (example: dividing an int by zero, reaching OpUnreachable, invalid casting, overflow/undeflow). When in safe mode, will modify the invalid usage.
khronos_validation.gpuav_shader_sanitizer = true
...
# =====================
# Detects data races on shared memory (Compute shaders only). Limitations: This does not handle mixing atomics and non-atomics on the same locations without a barrier in between, this could lead to false positives. This assumes barrier() is used for all shared memory synchronization.
khronos_validation.gpuav_shared_memory_data_race = true

# TraceRay SPIR-V instructions
# =====================
# Enable shader instrumentation on OpTraceRayKHR, OpHitObject*, OpReportIntersectionKHR, OpRayQueryInitializeKHR
khronos_validation.gpuav_validate_trace_ray = true
