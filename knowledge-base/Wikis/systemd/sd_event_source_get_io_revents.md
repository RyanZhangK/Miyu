...
Setting the I/O event mask to watch for to 0 does not mean that the event source will not be triggered anymore, as `EPOLLHUP` and `EPOLLERR` may be triggered even with a zero event mask. To temporarily disable an I/O event source use sd_event_source_set_enabled(3) with `SD_EVENT_OFF` instead.
...
`sd_event_io_handler_t()`, `sd_event_add_io()`, `sd_event_source_get_io_events()`, `sd_event_source_set_io_events()`, `sd_event_source_get_io_revents()`, `sd_event_source_get_io_fd()`, and `sd_event_source_set_io_fd()` were added in version 229.
