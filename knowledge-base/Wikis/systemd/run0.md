...
`--lightweight=`*`BOOLEAN`*  
Controls whether to activate the per-user service manager for the target user. By default (unset or set to `auto`), if the target user is "`root`" or a system user the per-user service manager is not activated as effect of the **run0** invocation, otherwise it is.
