...
`systemd-socket-activate` \[OPTIONS...\] *`COMMAND`* \[ARGS...\]
...
`-a`, `--accept`  
Launch an instance of the service program for each connection and pass the connection socket. May not be combined with `--now`.
...
`--now`  
Start the service program instantly, instead of waiting for a connection on the socket(s). May not be combined with `--accept`.
