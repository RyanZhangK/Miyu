...
Takes a boolean argument. Defaults to "`yes`". If "`no`", disables the generator entirely. `rd.systemd.verity=` is honored only in the initrd while `systemd.verity=` is honored by both the main system and in the initrd.

Added in version 233.

`veritytab=`, `rd.veritytab=`  
Takes a boolean argument. Defaults to "`yes`". If "`no`", causes the generator to ignore any devices configured in `/etc/veritytab`. `rd.veritytab=` is honored only in the initrd while `veritytab=` is honored by both the main system and in the initrd.
