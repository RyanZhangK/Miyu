...
`x-systemd.wants=`  
Configures a `Wants=` and an `After=` dependency between the created mount unit and another systemd unit, similar to the `x-systemd.requires=` option.
...
Validates mount constraint metadata of the mounted file system after mounting it. This ensures the systemd-validatefs@.service(8) service is pulled in by the mount unit.

Note that this option can only be used in `/etc/fstab`, and will be ignored when part of the `Options=` setting in a unit file. It is also implied for all partitions discovered by systemd-gpt-auto-generator(8).
