...
`sd_bus_query_sender_creds()` returns the credentials of the message *`m`*. The *`mask`* parameter is a combo of `SD_BUS_CREDS_*` flags that indicate which credential info the caller is interested in. See sd_bus_creds_new_from_pid(3) for a list of possible flags. First, this message checks if the requested credentials are attached to the message itself. If not, but the message contains the pid of the sender and the caller specified the `SD_BUS_CREDS_AUGMENT` flag, this function tries to figure out the missing credentials via other means (starting from the pid). If the PID is not available but the message has a sender, this function calls sd_bus_get_name_creds(3) to get the requested credentials. If the message has no sender (when a direct connection is used), this function calls sd_bus_get_owner_creds(3) to get the requested credentials. On success, the requested credentials are stored in *`creds`*. Ownership of the credentials object in *`creds`* is transferred to the caller and should be freed by calling sd_bus_creds_unref(3).
