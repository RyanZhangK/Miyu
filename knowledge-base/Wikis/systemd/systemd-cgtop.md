...
Resource usage is only accounted for control groups with the appropriate controllers turned on: "`memory`" controller for memory usage and "`io`" controller for disk I/O consumption. If resource monitoring for these resources is required, it is recommended to add the `MemoryAccounting=1` and `IOAccounting=1` settings in the unit files in question. See systemd.resource-control(5) for details.
