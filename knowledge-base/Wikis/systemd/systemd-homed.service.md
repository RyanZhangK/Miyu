...
`systemd-homed.service` also manages blob directories for each home directory it manages. See User Record Blob Directories for more details.
...
`SIGUSR1`  
Upon reception of the `SIGUSR1` process signal **systemd-homed** will reestablish its file watches on `/home/` and rescan the directory for home directories.
