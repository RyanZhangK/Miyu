...
Configures the key mapping table for the keyboard. `KEYMAP=` defaults to "`us`" if not set. As a special case, if "`@kernel`" is specified, no keymap will be loaded, i.e. the kernel's default keymap will be used. The `KEYMAP_TOGGLE=` can be used to configure a second toggle keymap and is by default unset.
