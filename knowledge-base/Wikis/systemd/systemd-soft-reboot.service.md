...
- On top of the above, templated units also need a configuration file for their slice, as they by default use a slice named after the non-templated part of the unit. For example, for a `foo@test.service` instance, a `system-foo.slice` unit may be added with the following content:
