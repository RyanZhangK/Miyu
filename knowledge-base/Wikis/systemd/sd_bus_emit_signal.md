...
`sd_bus_emit_signal_to()` and `sd_bus_emit_signal_tov()` are identical to `sd_bus_emit_signal()` and `sd_bus_emit_signalv()`, except that they can emit the signal to a single destination. Give *`destination`* as `NULL` to broadcast the signal.
...
`sd_bus_emit_signal()`, `sd_bus_emit_interfaces_added()`, `sd_bus_emit_interfaces_added_strv()`, `sd_bus_emit_interfaces_removed()`, `sd_bus_emit_interfaces_removed_strv()`, `sd_bus_emit_properties_changed()`, `sd_bus_emit_properties_changed_strv()`, were added in version 221.
