...
When these services are run, they ensure that PID 1 is replaced by the `/usr/lib/systemd/systemd-shutdown` binary which is then responsible for the actual shutdown. Before shutting down, this binary will try to unmount all remaining file systems (or at least remount them read-only), disable all remaining swap devices, detach all remaining storage devices and kill all remaining processes.
