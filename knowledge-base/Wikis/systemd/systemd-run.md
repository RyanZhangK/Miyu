...
`--pty-late`, `-T`  
Much like `--pty`, but the PTY communication only begins once the unit start is complete. This effectively means PTY input during `ExecStartPre=` of a service is not available while using `--pty-late`, while it is available using `--pty`, for most service types. However, if this option is used, access to the invoking TTY is available for password queries (see `--no-ask-password` below) during unit start-up.
...
`--background=`*`COLOR`*  
Change the terminal background color to the specified ANSI color as long as the session lasts. The color specified should be an ANSI X3.64 SGR background color, i.e. strings such as "`40`", "`41`", …, "`47`", "`48;2;…`", "`48;5;…`". See ANSI Escape Code (Wikipedia) for details.
