...
Notifications can specify actions that can be activated by the user. Actions whose name starts with ‘app.’ are assumed to be exported (without the ‘app.’ prefix) and will be activated via [D-Bus Activation](https://specifications.freedesktop.org/desktop-entry-spec/latest/ar01s08.html) using `ActivateAction()` method in the `org.freedesktop.Application` interface. An activation-token for [XDG Activation](https://gitlab.freedesktop.org/wayland/wayland-protocols/-/blob/main/staging/xdg-activation/xdg-activation-v1.xml) and an application-defined target is passed when activating the action. Other actions are activated by sending the [org.freedesktop.portal.Notification::ActionInvoked](#org-freedesktop-portal-notification-actioninvoked) signal to the application.
...
Some properties in [org.freedesktop.portal.Notification.AddNotification](#org-freedesktop-portal-notification-addnotification) may have options advertised by the server.
...
- `icon` (`v`)
  ...
  A serialized icon to add to the notification. The icon must pass [icon validation](icons.md) in order to be used. The format for serialized icon is a tuple (sv) with the following supported keys:

  - `themed` (`as`)

    A themed icon containing an array of strings with the icon names.

    This is the same format as a [serialized](https://docs.gtk.org/gio/method.Icon.to_string.html) [GThemedIcon](https://docs.gtk.org/gio/class.ThemedIcon.html) at the moment, but this interoperability may change in the future.

  - `bytes` (`ay`)

    The icon bytes into a variant bytes array.

    This is the same format as a [serialized](https://docs.gtk.org/gio/method.Icon.to_string.html) [GBytesIcon](https://docs.gtk.org/gio/class.BytesIcon.html) at the moment, but this interoperability may change in the future.
...
- `category` (`s`)
  ...
  - `im.received`
    ...
    - `im.reply-with-text`:
      ...
      The user response (`s`) will be placed as the second value in the parameter array of exported actions. For non-exported actions it will be placed as the third value in the parameter array of [org.freedesktop.portal.Notification::ActionInvoked](#org-freedesktop-portal-notification-actioninvoked).
  ...
  - `cellbroadcast.danger.presidential`
    ...
    Intended to be used to display nationwide or presidential warnings broadcasted by the cell network.
  ...
  - `cellbroadcast.public-safety`
    ...
    Intended to be used to display public safety warnings broadcasted by the cell network.
...
2.  The platform-data as vardict (`a{sv}`) containing an `activation-token` (`s`) for [XDG Activation](https://gitlab.freedesktop.org/wayland/wayland-protocols/-/blob/main/staging/xdg-activation/xdg-activation-v1.xml)
