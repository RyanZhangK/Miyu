...
Notifies the session of a request for clipboard content of the given mime type. The callee provides a serial to track the request, which any [org.freedesktop.portal.Clipboard.SelectionWrite](#org-freedesktop-portal-clipboard-selectionwrite) responses must use.

Once the caller is done handling the ‘SelectionTransfer’ request, they must call [org.freedesktop.portal.Clipboard.SelectionWriteDone](#org-freedesktop-portal-clipboard-selectionwritedone) with the corresponding request’s serial and whether the request completed successfully. If the request is not handled, the caller should respond by setting ‘success’ to ‘false’.
