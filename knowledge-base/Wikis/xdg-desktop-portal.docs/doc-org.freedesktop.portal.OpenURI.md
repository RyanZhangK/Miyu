...
Note that `file://` URIs are explicitly not supported by this method. To request opening local files, use [org.freedesktop.portal.OpenURI.OpenFile](#org-freedesktop-portal-openuri-openfile).
...
uri  
A URI, as described in RFC 3986, to open
...
Asks to open a local file or directory.
...
fd  
File descriptor for the file or directory to open
...
Asks to open the directory containing a local file or directory represented by `fd` in the file browser, possibly selecting it. This is implemented using the `org.freedesktop.FileManager1.ShowItems()` D-Bus API and if that service doesn’t exist falls back to the `OpenURI` method on the directory of the file or directory.
...
fd  
File descriptor for a file or directory
