...
Under Wayland, it should have the form `wayland:<HANDLE>`, where `<HANDLE>` is a surface handle obtained with the [xdg_foreign](https://gitlab.freedesktop.org/wayland/wayland-protocols/-/blob/main/unstable/xdg-foreign/xdg-foreign-unstable-v2.xml) protocol. For example, `wayland:~12l9jdl.-a`.
