# Scripts / Desc / Xdg Icon Resource
...
install
Installs the icon file indicated by \ to the desktop icon system under the name \. Icon names do not have an extension. If \ is not provided the name is derived from \. The icon file must have *.png*, *.svg*, or *.xpm* as extension. If a corresponding *.icon* file exists in the same location as \ it will be installed as well.

uninstall
Removes the icon indicated by \ from the desktop icon system. Note that icon names do not have an extension.
...
`xdg-desktop-icon(1)`, `xdg-desktop-menu(1)`, `xdg-mime(1)`, `xdg-utils-common(7)`, [Icon theme specification](http://www.freedesktop.org/wiki/Specifications/icon-theme-spec/)
