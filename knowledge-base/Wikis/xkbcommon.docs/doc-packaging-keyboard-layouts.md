...
1. `$HOME/.config/xkb` (*user* path)
2. `$HOME/.xkb` (alternative *user* path)
...
3. Install the XKB files in:
   `<XKB_CONFIG_UNVERSIONED_EXTENSIONS_PATH>//<KCCGST_DIR>`, where:
...
4. Install the *registry file* at:
   `<XKB_CONFIG_UNVERSIONED_EXTENSIONS_PATH>//rules/evdev.xml`.
