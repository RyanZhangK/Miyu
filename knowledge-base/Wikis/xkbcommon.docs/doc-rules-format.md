...
[RMLVO][RMLVO] configuration (usually `evdev`). Then the following
files are parsed and their results are collected sequentially to form the final
[rule sets][]:

*Optional:* for each include path ``, parse and append the [rule sets]
of the file `/rules/<ruleset>.pre`, if it exists. Multiple such files may
...
*Optional:* for each include path ``, parse and append the [rule sets]
of the file `/rules/<ruleset>.post`, if it exists. Multiple such files
...
```
! include /rules/<ruleset>.pre  // only if defined
…
! include /rules/<ruleset>.pre  // only if defined

! include <ruleset>                             // main rules file

! include /rules/<ruleset>.post // only if defined
…
! include /rules/<ruleset>.post // only if defined
```
...
### Example: layout-specific options
