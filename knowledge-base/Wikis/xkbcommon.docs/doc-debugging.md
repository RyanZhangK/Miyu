...
1. Create a directory for the custom configuration, e.g. `$HOME/xkb-test`. Note that
   in order to test it safely, it should *not* be one of the locations that
   [xkbcommon searches][user-configuration locations].
2. Create the relevant sub-directories and files, e.g. `$HOME/xkb-test/symbols/my-layout`.
3. Test if your changes *compile* successfully:

   ```bash
   xkbcli compile-keymap --include $HOME/xkb-test --include-defaults --test --layout my-layout
   ```
...
4. Test if it *behaves* correctly. Note that you may need to add your user to the
   ...
   ```bash
   xkbcli interactive-evdev --include $HOME/xkb-test --include-defaults --enable-compose --layout my-layout
   ```
...
6. If everything works as expected, it is time to test in real conditions:
   - Wayland: , move `$HOME/xkb-test` to one of the [user-configuration locations],
   ...
   - X11: Unfortunately Xorg does not support alternative paths, so the next best
     ...
     ```bash
     xkbcli compile-keymap --include $HOME/xkb-test --include-defaults --layout my-layout \
       | xkbcomp - $DISPLAY
    ```
